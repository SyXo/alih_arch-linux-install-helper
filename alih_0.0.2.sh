...
#Create config file and insert protected data for wpa_supplicant (wireless network)
echo "ctrl_interface=/run/wpa_supplicant\nupdate_config=1\n\n" >> /etc/wpa_supplicant/wpa_supplicant.conf
wpa_passphrase $MY-ESSID $MY-PASSPHRASE >> /etc/wpa_supplicant/wpa_supplicant.conf

#Test wpa_supplicant config
wpa_supplicant -c /etc/wpa_supplicant/wpa_supplicant.conf -i $my-interface-name

#Run wpa_supplicant in the background
wpa_supplicant -B -c /etc/wpa_supplicant/wpa_supplicant.conf -i $my-interface-name

#Start and enable dhcp service to obtain an IP and check it for errors
systemctl start dhcpcd && systemctl enable dhcpcd && systemctl status dhcpcd
